...
UI_TEST_STEP_DELAY = float(os.environ.get("UI_TEST_STEP_DELAY", "1.0"))
...
def pause(seconds=None):
    time.sleep(UI_TEST_STEP_DELAY if seconds is None else seconds)


def slow_driver_actions(driver):
    original_get = driver.get

    def get_with_pause(url):
        pause()
        original_get(url)
        pause()

    driver.get = get_with_pause
    return driver
...
def create_driver():
    try:
        ...
        return slow_driver_actions(driver)
    except Exception:
