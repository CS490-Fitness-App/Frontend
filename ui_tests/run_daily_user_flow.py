...
def fill_activity_log(driver):
    ...
    notes.clear()
    notes.send_keys("Automated daily UI flow test entry.")
    ...
    driver.get(f"{BASE_URL}/activity-logger")
    wait_for(driver, lambda d: "activity logger" in visible_text(d), 20)
    wait_for(driver, EC.presence_of_element_located((By.XPATH, "//button[contains(normalize-space(), \"Delete Today's Log\")]")), 20)


def delete_today_log(driver):
    driver.get(f"{BASE_URL}/activity-logger")
    wait_for(driver, lambda d: "activity logger" in visible_text(d), 20)

    delete_button = wait_for_clickable(driver, By.XPATH, "//button[contains(normalize-space(), \"Delete Today's Log\")]")
