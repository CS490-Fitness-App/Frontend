...
def login_as_coach(driver):
    if not UI_TEST_COACH_EMAIL or not UI_TEST_PASSWORD:
        raise RuntimeError("Set UI_TEST_COACH_EMAIL and UI_TEST_PASSWORD before running the coach flow.")
...
def test_coach_message_client(driver):
    ...
    wait_for(
        driver,
        lambda d: "/chats" in d.current_url,
        20,
    )
    wait_for(driver, lambda d: "message" in visible_text(d) or "conversation" in visible_text(d), 20)
